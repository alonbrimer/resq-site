...
class JSON_API_Core_Controller {
  ...
  public function getUsers() {
    $result = wp_remote_get("http://production-resq.rhcloud.com/resq-server/users/get");
    $result =  (json_decode($result['body']));
    return $result;
  }

    public function getRescalls() {
        $result = wp_remote_get("http://production-resq.rhcloud.com/resq-server/rescalls/get");
        $result =  (json_decode($result['body']));
        return $result;
    }

    public function deleteRescall(){
        $arg = array ( 'method' => 'DELETE');
        $id = $_GET['id'];
        $result = wp_remote_get("http://production-resq.rhcloud.com/resq-server/rescalls/delete/" . $id, $arg);
    }

    public function  deactivateUser(){
        $id = $_GET['id'];
        $result = wp_remote_post("http://production-resq.rhcloud.com/resq-server/users/deactivate/" . $id);
    }

    public function  activateUser(){
        $id = $_GET['id'];
        $result = wp_remote_post("http://production-resq.rhcloud.com/resq-server/users/activate/" . $id);
    }

    public function  getUser(){
        $id = $_GET['id'];
        $result = wp_remote_get("http://production-resq.rhcloud.com/resq-server/users/get/" .$id);
        $result =  (json_decode($result['body']));
        return $result;
    }

public function  getRescall(){
    $id = $_GET['id'];
    $result = wp_remote_get("http://production-resq.rhcloud.com/resq-server/rescalls/get/" .$id);
    $result =  (json_decode($result['body']));
    return $result;
}
